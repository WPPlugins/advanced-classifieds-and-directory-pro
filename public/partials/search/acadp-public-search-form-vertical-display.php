			...
			<div class="form-group">
				...
				<?php 
					acadp_dropdown_terms( array(
						...
						'selected'          => isset( $_GET['location'] ) ? (int) $_GET['location'] : $general_settings['base_location']
        			) );
				?>
			</div>
        <?php endif; ?>
        ...
        <?php if( $can_search_by_custom_fields ) : ?>
        	 <!-- Custom fields -->
       		<div id="acadp-custom-fields-search-<?php echo $id; ?>" class="acadp-custom-fields-search" data-style="<?php echo $style; ?>">
  				<?php do_action( 'wp_ajax_acadp_custom_fields_search', isset( $_GET['category'] ) ? (int) $_GET['category'] : 0, $style ); ?>
			</div>
        <?php endif; ?>        
